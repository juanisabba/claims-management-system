...
import { Claim, ClaimSummary, Damage } from '../../core/models/claim.model';
...
export class HttpClaimRepository implements ClaimRepository {
  ...
  getClaims(limit: number = 10, offset: number = 0): Observable<PaginatedResult<ClaimSummary>> {
    return this.http
      .get<PaginatedResult<ClaimSummaryApiResponse>>(`${this.apiUrl}/claims`, {
        params: { limit: limit.toString(), offset: offset.toString() },
      })
      .pipe(
        map((response) => ({
          ...response,
          data: response.data.map(ClaimMapper.fromSummaryApi),
        })),
  ...
  getDamages(
    ...
    return this.http
      .get<PaginatedResult<DamageApiResponse>>(`${this.apiUrl}/claims/${claimId}/damages`, {
        params: { limit: limit.toString(), offset: offset.toString() },
      })
      .pipe(
        map((response) => ({
  ...
  getClaimById(id: string): Observable<Claim> {
    return this.http
      .get<ClaimApiResponse>(`${this.apiUrl}/claims/${id}`)
  ...
  createClaim(claim: { title: string; description: string }): Observable<Claim> {
    return this.http
      .post<ClaimApiResponse>(`${this.apiUrl}/claims`, claim)
      .pipe(map(ClaimMapper.fromApi), catchError(this.handleError));
  }

  updateClaim(id: string, claim: { title: string; description: string }): Observable<Claim> {
    return this.http
      .patch<ClaimApiResponse>(`${this.apiUrl}/claims/${id}`, claim)
      .pipe(map(ClaimMapper.fromApi), catchError(this.handleError));
  }

  addDamage(claimId: string, damage: Omit<Damage, 'id'>): Observable<Claim> {
    return this.http
      .post<ClaimApiResponse>(`${this.apiUrl}/claims/${claimId}/damages`, damage)
      .pipe(map(ClaimMapper.fromApi), catchError(this.handleError));
  }
  ...
  updateDamage(
    ...
    return this.http
      .patch<ClaimApiResponse>(`${this.apiUrl}/claims/${claimId}/damages/${damageId}`, damage)
      .pipe(map(ClaimMapper.fromApi), catchError(this.handleError));
  }
  ...
  updateStatus(id: string, status: ClaimStatus): Observable<Claim> {
    return this.http
      .patch<ClaimApiResponse>(`${this.apiUrl}/claims/${id}`, { status })
      .pipe(map(ClaimMapper.fromApi), catchError(this.handleError));
  }
